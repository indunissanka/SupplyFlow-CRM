...
// In Cloudflare Workers, these would be set as environment variables or bindings
...
export function setMongoDBEnv(config: MongoDBEnv) {
  env = config;
}
...
export function createMongoDBQueryable(db: Db): MongoDBQueryable {
  return {
    collection<T extends Document>(name: string): Collection<T> {
    ...
    async batch(statements: MongoDBPreparedStatement[]): Promise<any[]> {
      const results = [];
...
export function sqlWhereToMongoFilter(whereClause: string, params: any[]): Filter<Document> {
...
// ObjectId utilities
export function toObjectId(id: string | number | ObjectId): ObjectId {
  if (id instanceof ObjectId) return id;
  if (typeof id === 'string' && ObjectId.isValid(id)) return new ObjectId(id);
  if (typeof id === 'number') return new ObjectId(id.toString());
