...
async function syncShippingMilestones(db: D1Database, ownerEmail: string) {
  ...
  for (const row of results) {
    ...
    if (computedStatus && row.status !== computedStatus) {
      scheduleUpdates.push(updateSchedule.bind(computedStatus, row.id, ownerEmail, computedStatus));
    }
    // Use highest of date-computed or manually stored status for order/invoice propagation
    const storedRank = row.status ? (shippingStatusRank[row.status] ?? 0) : 0;
    const computedRank = computedStatus ? (shippingStatusRank[computedStatus] ?? 0) : 0;
    const effectiveStatus = computedRank >= storedRank ? computedStatus : row.status;
    if (!effectiveStatus) continue;
    if (row.order_id) applyStatus(orderStatuses, row.order_id, effectiveStatus);
    if (row.invoice_id) applyStatus(invoiceStatuses, row.invoice_id, effectiveStatus);
  }
