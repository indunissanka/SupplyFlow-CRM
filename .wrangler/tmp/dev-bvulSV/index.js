...
app.post("/api/sample_shipments", async (c) => {
  const body = await c.req.json();
  const result = await c.env.DB.prepare(
    `INSERT INTO sample_shipments (company_id, product_id, document_id, receiving_address, phone, quantity, waybill_number, courier, status, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(
    body.company_id ?? null,
...
async function ensureSchema(db) {
  ...
  const sampleColumns = await db.prepare("PRAGMA table_info(sample_shipments)").all();
...
// .wrangler/tmp/bundle-dWYj4K/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-dWYj4K/middleware-loader.entry.ts
