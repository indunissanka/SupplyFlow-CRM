...
var updatableFields = {
  ...
  orders: ["company_id", "contact_id", "quotation_id", "status", "total_amount", "currency", "reference"],
...
var schemaIndexStatements = schemaStatements.filter(
  (statement) => statement.startsWith("CREATE INDEX") || statement.startsWith("CREATE UNIQUE INDEX")
);
var schemaCreateStatements = /* @__PURE__ */ new Map();
schemaStatements.forEach((statement) => {
  const match2 = statement.match(/^CREATE TABLE IF NOT EXISTS (\w+)/);
  if (match2) {
    schemaCreateStatements.set(match2[1], statement);
  }
});
...
app.get("/api/:table", async (c) => {
  ...
  const ownerEmail = c.get("ownerEmail");
  if (table3 === "orders" || table3 === "invoices" || table3 === "shipping_schedules") {
    await syncShippingMilestones(c.env.DB, ownerEmail);
  }
  const rows = await fetchRows(c.env.DB, table3, ownerEmail);
  return c.json({ rows });
});
app.post("/api/account/update", async (c) => {
  ...
  for (const table3 of ownerEmailTables) {
    const existing = await c.env.DB.prepare(`SELECT 1 FROM ${table3} WHERE owner_email = ? LIMIT 1`).bind(nextEmail).first();
    if (existing) {
      return c.json({ error: "Email already in use" }, 409);
    }
  }
  const updates = ownerEmailTables.map(
    (table3) => c.env.DB.prepare(`UPDATE ${table3} SET owner_email = ? WHERE owner_email = ?`).bind(nextEmail, ownerEmail)
  );
...
app.put("/api/:table/:id", async (c) => {
  ...
  if (entries.length) {
    ...
    const stmt = c.env.DB.prepare(`UPDATE ${table3} SET ${setParts} WHERE id = ? AND owner_email = ?`).bind(...values, id, ownerEmail);
    const result = await stmt.run();
    changes = result.meta.changes;
  }
  const entityType = entityTypes[table3];
  if (entityType && tags.length) {
    await c.env.DB.prepare("DELETE FROM tag_links WHERE entity_type = ? AND entity_id = ? AND owner_email = ?").bind(entityType, id, ownerEmail).run();
    await attachTags(c.env.DB, ownerEmail, entityType, id, tags);
  }
  if (table3 === "shipping_schedules") {
    await syncShippingMilestones(c.env.DB, ownerEmail);
  }
...
app.delete("/api/:table/:id", async (c) => {
  ...
  await c.env.DB.prepare(`DELETE FROM ${table3} WHERE id = ? AND owner_email = ?`).bind(id, ownerEmail).run();
...
app.post("/api/orders", async (c) => {
  ...
  const result = await c.env.DB.prepare(
    `INSERT INTO orders (company_id, contact_id, quotation_id, status, total_amount, currency, reference, owner_email)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(
    body.company_id ?? null,
...
app.post("/api/notes", async (c) => {
  ...
  const result = await c.env.DB.prepare(
    `INSERT INTO notes (entity_type, entity_id, body, author, note_date, owner_email)
     VALUES (?, ?, ?, ?, ?, ?)`
  ).bind(body.entity_type, body.entity_id ?? null, body.body, body.author ?? null, body.note_date ?? null, ownerEmail).run();
  const id = result.meta.last_row_id;
...
app.post("/api/tags", async (c) => {
  const ownerEmail = c.get("ownerEmail");
  const body = await c.req.json();
  const result = await c.env.DB.prepare(`INSERT INTO tags (name, color, owner_email) VALUES (?, ?, ?)`).bind(body.name, body.color ?? "#2563eb", ownerEmail).run();
  return c.json({ id: result.meta.last_row_id });
});
...
app.post("/api/upload", async (c) => {
  ...
  for (const file of files) {
    ...
    const stmt = c.env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      RETURNING id, title, storage_key as key
    `).bind(companyId, contactId, invoiceId, docTypeId, title2, key, contentType, body.byteLength, ownerEmail);
...
app.get("/api/files/:key{.+}", async (c) => {
  ...
  const record = await c.env.DB.prepare("SELECT id FROM documents WHERE storage_key = ? AND owner_email = ?").bind(key, ownerEmail).first();
  if (!record) {
    return c.notFound();
  }
...
var src_default = app;
async function hasSingleColumnUniqueIndex(db, table3, column) {
  const indexes = await db.prepare(`PRAGMA index_list(${table3})`).all();
  for (const idx of indexes.results ?? []) {
    if (!idx.unique || !idx.name) continue;
    const info3 = await db.prepare(`PRAGMA index_info(${idx.name})`).all();
    const columns = info3.results?.map((row) => row.name) ?? [];
...
__name(hasSingleColumnUniqueIndex, "hasSingleColumnUniqueIndex");
async function rebuildOwnerScopedTable(db, table3) {
  const createTemplate = schemaCreateStatements.get(table3);
  if (!createTemplate) return false;
  const columnsInfo = await db.prepare(`PRAGMA table_info(${table3})`).all();
  const columns = columnsInfo.results?.map((row) => row.name) ?? [];
  if (!columns.length) return false;
  const createSql = createTemplate.replace(
    `CREATE TABLE IF NOT EXISTS ${table3}`,
    `CREATE TABLE ${table3}_new`
  );
  const quotedColumns = columns.map((col) => `"${col}"`).join(", ");
  const selectColumns = columns.map((col) => col === "owner_email" ? "COALESCE(owner_email, '')" : `"${col}"`).join(", ");
  await db.prepare(`DROP TABLE IF EXISTS ${table3}_new`).run();
  await db.prepare("PRAGMA foreign_keys = OFF").run();
  await db.prepare(createSql).run();
  await db.prepare(`INSERT INTO ${table3}_new (${quotedColumns}) SELECT ${selectColumns} FROM ${table3}`).run();
  await db.prepare(`DROP TABLE ${table3}`).run();
  await db.prepare(`ALTER TABLE ${table3}_new RENAME TO ${table3}`).run();
  await db.prepare("PRAGMA foreign_keys = ON").run();
  return true;
}
__name(rebuildOwnerScopedTable, "rebuildOwnerScopedTable");
async function rebuildOwnerUniqueTables(db) {
  const targets = [
  ...
  for (const target of targets) {
    const hasTable = await db.prepare(`PRAGMA table_info(${target.table})`).all();
...
__name(rebuildOwnerUniqueTables, "rebuildOwnerUniqueTables");
async function ensureSchema(db) {
  ...
  for (const table3 of ownerEmailTables) {
    const columns = await db.prepare(`PRAGMA table_info(${table3})`).all();
    const names = new Set(columns.results?.map((c) => c.name));
    if (names.size && !names.has("owner_email")) {
      await db.prepare(`ALTER TABLE ${table3} ADD COLUMN owner_email TEXT`).run();
    }
  }
...
function computeShippingMilestoneStatus(row, nowMs) {
...
__name(computeShippingMilestoneStatus, "computeShippingMilestoneStatus");
async function syncShippingMilestones(db, ownerEmail) {
  ...
  const orderStatuses = /* @__PURE__ */ new Map();
  const invoiceStatuses = /* @__PURE__ */ new Map();
  const updateSchedule = db.prepare(
    `UPDATE shipping_schedules
     SET status = ?, updated_at = CURRENT_TIMESTAMP
     WHERE id = ? AND owner_email = ? AND (status IS NULL OR status <> ?)`
  );
  const applyStatus = /* @__PURE__ */ __name((target, id, status) => {
...
__name(syncShippingMilestones, "syncShippingMilestones");
async function getStats(db, ownerEmail) {
  const result = {};
  for (const table3 of ["companies", "contacts", "orders", "quotations", "invoices", "tasks"]) {
    const row = await db.prepare(`SELECT COUNT(*) as count FROM ${table3} WHERE owner_email = ?`).bind(ownerEmail).first();
    result[table3 === "orders" ? "openOrders" : table3] = row?.count ?? 0;
  }
  return result;
}
__name(getStats, "getStats");
async function fetchRows(db, table3, ownerEmail) {
  const orderColumn = table3 === "tags" || table3 === "doc_types" ? "created_at" : "updated_at";
  let query = `SELECT * FROM ${table3} WHERE owner_email = ? ORDER BY ${orderColumn} DESC LIMIT 50`;
  let params = [ownerEmail];
  switch (table3) {
    case "contacts":
...
async function attachTags(db, ownerEmail, entityType, entityId, tags) {
  if (!entityId || !tags.length) return;
  const inserts = tags.map(
    (tagId) => db.prepare("INSERT OR IGNORE INTO tag_links (tag_id, entity_type, entity_id, owner_email) VALUES (?, ?, ?, ?)").bind(tagId, entityType, entityId, ownerEmail)
  );
  if (inserts.length) {
...
async function getPipeline(db, ownerEmail) {
...
async function getActivity(db, ownerEmail) {
...
// .wrangler/tmp/bundle-7TDr0E/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-7TDr0E/middleware-loader.entry.ts
